function plothisf_revocable(client,imposter,mate_imp,flag,bitlength,maxscore,maxsum)
...
collection_dist_mate_incorrect=mate_imp;
...
collection_dist_incorrect3 = zeros(1,length(X_vect)+3);
...
collection_dist_incorrect3(1,1:length(X_vect)) = hist(collection_dist_mate_incorrect,X_vect);
...
collection_dist_incorrect3(1,length(X_vect)+1) = mean(collection_dist_mate_incorrect);
...
collection_dist_incorrect3(1,length(X_vect)+2) = var(collection_dist_mate_incorrect);
...
collection_dist_mate_incorrect_tot{index_save,index_eigenspace} = collection_dist_incorrect3;
...
Y3 = collection_dist_mate_incorrect_tot{l,end}(1:end-3); % get the Y axis values of the different bins of the histogram for different users
...
mean3(1) = collection_dist_mate_incorrect_tot{l,end}(end-2); % get the mean distance for different users
...
var3(1) = collection_dist_mate_incorrect_tot{l,end}(end-1); % get the distance variance for different users
...
plot(X_vect,Y1,'r-',X_vect,Y2,'b-h',X_vect,Y3,'m-o','LineWidth',3.5);

xlabel('Normalized Matching Scores', 'FontSize', 15)
ylabel('Frequency', 'FontSize', 15)
title('Genuine, Imposter & Mated-Imposter: LFW10', 'FontSize', 15)

set(gcf,'color',[1 1 1])
lgd = legend(strcat('Genuine (mean: ',num2str(mean1),' | ','var: ',...
    num2str(var1),')'),strcat('Imposter (mean: ',num2str(mean2),' | ','var: ',num2str(var2),')')...
,strcat('Mated-Imposter (mean: ',num2str(mean3),' | ','var: ',num2str(var3),')'));
% ,strcat('Mated-Genuine (mean: ',num2str(mean3),' | ','var: ',num2str(var3),')'));
lgd.FontSize = 11;
